...
KernelObject::rdead = "Subkernel connected through `1` appears dead."
KernelObject::noid = "No parallel kernel with ID `1` found."
...
Module[{new, newid=OptionValue[KernelID], res, fails},
...
checkVersion[sub_, {subKernel_, subLanguageVersion_, subMathVersion_}] :=
	Which[
		...
		!TrueQ[subKernel], (* False or junk *)
			...
			$Failed,
		subLanguageVersion === Null || subLanguageVersion < Parallel`Private`$ParallelLanguageVersion,
			Needs["Parallel`OldClient`"];
			Parallel`OldClient`initOldKernel[sub, subLanguageVersion];
			True,
		subLanguageVersion > Parallel`Private`$ParallelLanguageVersion, (* in the future, not supported *)
...
Module[{res},
	...
	If[ res===$Failed, Message[KernelObject::rdead, kernel]; CloseError[kernel] ];
	rdF[kernel];
	rqueue[kernel];
	kernel
]
...
KernelFromID[id_Integer] := (If[processor[id]===$Failed, Message[KernelObject::noid, id]]; processor[id])
...
kernel/: Write[k_kernel, expr_] := Catch[linkWrite[k, expr], dead, (Message[KernelObject::rdead, k]; CloseError[k]; $Failed)&]
...
sendCatch[kernel_kernel, expr_] := Catch[send[kernel,expr], dead, (Message[KernelObject::rdead, kernel]; $Failed)&]
...
receiveCatch[k_, args___] := Catch[receive[k, args], dead, (Message[KernelObject::rdead, k]; $Failed)&]
...
poll[kernel_] :=
 AbortProtect[ Catch[ 
    ...
    ], dead, (Message[KernelObject::rdead, kernel]; CloseError[kernel]; $Failed)& ];
...
(* read from all kernels until each delivers a result matching patt *)
...
SuspendPacket/: PacketHandler[SuspendPacket[Null], kernel_] := (
	Message[KernelObject::rdead, subKernel[kernel]];
	CloseError[kernel]
)
...
kernel/: Abort[kernel_kernel] :=
  Module[{res = Null},
     ...
     CheckAbort[ (* if we get aborted ourselves, mark kernel as failed *)
      ...
      ], dead, (Message[KernelObject::rdead, kernel]; CloseError[kernel]; res=$Failed)& ]; (* While and Catch *)
      If[ res === $Failed, Return[res] ]; (* bad luck *)
      If[ res =!= $Aborted, (* clean up if not done already *)
      	res = kernelFlush[subKernel[kernel]];
        If[res === $Failed, Message[KernelObject::rdead, kernel]; CloseError[kernel]; Return[res] ];
