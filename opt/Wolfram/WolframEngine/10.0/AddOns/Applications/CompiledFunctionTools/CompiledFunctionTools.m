...
Void::usage = "Void represents an uninitialized type, or a function that returns no result."
...
ToCompiledProcedureLocalized[
 ...
 Module[{info, setup, result, insts, code, cons}, 
 	...
 	clearTensorTable[];
  	info = getCompiledInfo[args, regs, Drop[setupCode,-1], rtFlags];
...
getInstruction[line_, {CCRE, regs_, rego_}] := 
 Instruction[Re, toRegister[{Real, rego}], 
  {toRegister[{Complex, regs}]}]

getInstruction[line_, {CCIM, regs_, rego_}] := 
 Instruction[Im, toRegister[{Real, rego}], 
  {toRegister[{Complex, regs}]}]
