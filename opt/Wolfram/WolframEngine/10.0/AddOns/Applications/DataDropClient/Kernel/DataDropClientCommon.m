...
getBinURL[id_String]:=With[{shortid=getShortBinID[id]},
	urlbase<>shortid
]

getBinURL[id_String,"Long"]:=With[{uuid=getBinID[id]},
	urlbase<>uuid
]
...
storebin[id_,res_Association]:=Module[{uuid,shortid},
	If[Quiet[KeyExistsQ[res,"BinIDs"]],
		...
		storeRecent[uuid,importdrop/@res["Recent"]];
...
storeBinID0[id_,key_, res_]:=If[MatchQ[res,_Association|_List],
		If[KeyExistsQ[res,"UUID"],
			datadropclientcache[{"DatabinIDs", id}]=datadropclientcache[{"DatabinIDs", res["UUID"]}]=res;
			If[key===All,
				res,
...
makedatabin[shortid_,id_,name_]:=(
	datadropclientcache[{"DatabinIDs", id}]=datadropclientcache[{"DatabinIDs", shortid}]=Association[{"UUID"->id, "ShortID"->shortid,"Name"->name}];
	datadropclientcache[{"DatabinStats", id}]=datadropclientcache[{"DatabinStats", shortid}]=Association[{
		"Entries"->0, "Size"->0,"LatestTimestamp"->None,"CreationDate"->timezoneconvert[DateObject[]],"Creator"->$WolframID,"Owner"->$WolframID}];
...
timezoneconvert[x__]:=TimeZoneConvert[x]
