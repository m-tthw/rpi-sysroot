...
(*************** Options/Settings *************)
System`Databin/:HoldPattern[Options[HoldPattern[System`Databin][id_,___]]]:=databinsettings[id]

databinsettings[id_]:=getBinSettings[id]

System`Databin/:HoldPattern[SetOptions[HoldPattern[System`Databin][id_,___], rest___]]:=setdatabinsettings[id, rest]

setdatabinsettings[id_, as_Association]:=With[{res=apifun["EditBinSettings",Join[Association[{"Bin"->id}],as]]},
	If[KeyExistsQ[as,"Name"],
		getBinID0[id,"Name"]
	];
	res
]
...
(************* UpValues *******************)
databinUpValueFunctionsMap=Join[{BarChart, BoxWhiskerChart, DateListPlot, DistributionChart, 
...
TimeSeries, TimeSeriesAggregate, TimeSeriesMap, TimeSeriesMapThread, 
...
RandomProcesses`TemporalDataDump`$DescriptiveStatisticsList,
 ...
 {TimeValue, EffectiveInterest, Cashflow, ListPlay, ListAnimate, 
  SampledSoundList, Interpolation, Standardize, Ratios, Accumulate, 
...
mapUpValue[f_]:=Databin/:f[before___,db_Databin,after___]:=Map[f[before,#,after]&,db["Data"]]
mapUpValue/@Complement[databinUpValueFunctionsMap,databinUpValueFunctionsThread,excluded]

threadUpValue[f_]:=Databin/:f[before___,db_Databin,after___]:=f[before,Values[db["Data"]],after]
threadUpValue/@databinUpValueFunctionsThread

databinUpValueFunctionsCustom={Get}
Databin/:Get[db_Databin]:=db["Data"]
...
Catch[With[{id=databinID[databin], icon=databinIcon},
	If[StringQ[id],
		With[{fullid=loaddatabin[id]},
			If[$VersionNumber>=10&&StringQ[fullid],
				BoxForm`ArrangeSummaryBox[
					(* Head *)Databin, 
					(* Interpretation *)databin, 
					(* Icon *)icon, 
					(* Column or Grid *){	BoxForm`MakeSummaryItem[{"Short ID: ", getShortBinID[fullid]}, form],
					BoxForm`MakeSummaryItem[{"Entries: ", Dynamic[Lookup[Replace[
							datadropclientcache[{"DatabinStats", fullid}],Except[_Association]->{},{0}],"Entries",0]]}, form]}, 
					(* Plus Box Column or Grid *){
						BoxForm`MakeSummaryItem[{"URL: ", Hyperlink[getBinURL[fullid]]}, form],
						BoxForm`MakeSummaryItem[{"UUID: ", fullid}, form],
						BoxForm`MakeSummaryItem[{"Name: ", getBinName[id]}, form],
						BoxForm`MakeSummaryItem[{"Creation date: ", getCreationDate[fullid]}, form],
						BoxForm`MakeSummaryItem[{"Creator: ", getCreator[id,"Creator"]}, form],
						BoxForm`MakeSummaryItem[{"Owner: ", getCreator[id,"Owner"]}, form],
						BoxForm`MakeSummaryItem[{"Size: ", Quantity[Dynamic[
							Round[Lookup[Replace[datadropclientcache[{"DatabinStats", fullid}],Except[_Association]->{},{0}],"Size",0]/1000],1/10],"Kilobytes"]}, form],
						BoxForm`MakeSummaryItem[{"Latest date: ", Dynamic[
							Lookup[Replace[datadropclientcache[{"DatabinLatest", fullid}],Except[_Association]->{},{0}],"Timestamp",Missing[]]]}, form], 
						BoxForm`MakeSummaryItem[{"Latest: ", Dynamic[
							Lookup[Replace[datadropclientcache[{"DatabinLatest", fullid}],Except[_Association]->{},{0}],"Data",Missing[]]]}, form]
						(*,
						BoxForm`MakeSummaryItem[{"Sample: ", Dynamic[Replace[datadropclientcache[{"DatabinRecentEventSeries", fullid}],Except[_TemporalData]->Missing[],{0}]]}, form] *)
					}, 
					form]
					,
				ToBoxes[$Failed]
