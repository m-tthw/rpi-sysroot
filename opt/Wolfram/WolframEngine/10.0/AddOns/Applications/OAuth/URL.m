...
(* $Id: URL.m,v 1.16 2013/11/15 21:53:39 riccardod Exp $ *)
...
ClearAll[associationJoin, associationRules, Destroy]

(* Internal utilities *)
associationJoin[ass___Association] := Association[Flatten[associationRules /@ {ass}]];
associationRules[ass_Association] := Rule @@@ Transpose[{Keys[ass], Values[ass]}];
...
(* URLEncode *)
URLEncode[s_String, OptionsPattern[]] := StringReplace[s, {
	    " " -> "+", 
...
(* URLQueryEncode *)
URLQueryEncode[args_Association, options___Rule] := URLQueryEncode[associationRules[args], options];
...
joinQueries[queries_, options___] := Flatten[Map[Replace[#, {
	...
	a_Association :> associationRules[a]
}] &, queries]];
