...
CInclude::usage = "CInclude[ header] is a symbolic representation of a a preprocessor include statement."
...
CFor::usage = "CFor[ init, test, incr, body] is a symbolic representation of a for loo[."
...
CStruct::usage = "CCStruct name, members] is a symbolic representation of a struct. CStruct[ name] declares a struct without specifying the contents. CStruct[ None, members] does not give the struct a name."
...
(*
 Toplevel function
*)
Options[ToCCodeString] = Options[GenerateCode] = {Embedded -> False, "Indent" -> None}
ToCCodeString[ arg_, opts:OptionsPattern[]] := 
	If[ IsCExpression[arg] || ListQ[ arg] || MatchQ[ arg, _CComment ], 
...
GenerateCode[CDo[ body_, test_], opts:OptionsPattern[]] := 
	Module[ {},
		StringJoin[
			"do", StringTrim[GenerateCode[CBlock[body], opts]], " while( ", GenerateCode[test, opts], ");"
		]
	]
...
GenerateCode[CFor[ init_, test_, incr_, body_], opts:OptionsPattern[]] := 
	Module[ {},
		StringJoin[
			"for( ",
			If[ListQ[init],
				Riffle[StringTrim[GenerateCode[#, opts]& /@ init, (Whitespace | ";") ...], ", "], 
			...
			"; ",
			If[ListQ[incr],
				Riffle[StringTrim[GenerateCode[#, opts]& /@ incr, (Whitespace | ";") ...], ", "],
...
formatSwitchArgument[ {label_, args__}, opts:OptionsPattern[]] :=
	StringJoin[
		Which[
			StringTrim[label] === "default", "default:",
