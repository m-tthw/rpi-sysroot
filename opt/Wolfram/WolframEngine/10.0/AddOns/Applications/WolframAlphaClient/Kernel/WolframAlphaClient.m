...
$AlphaQueryBaseURLs = {
	...
	"http://www.test2.wolframalpha.com/api/v1/" -> "Test2",
...
$MostlyInputFormTypesetHeads = "Placeholder" | "Out" | "Quantity" | "Entity" | "EntityProperty" | "WolframAlphaClient`Private`rawCompressedBoxes";
...
Internal`MWACompute[type_String, argsIn_, OptionsPattern[]] := 
...
, timeout, args=argsIn},
		...
		argscompressed = Block[{$ContextPath = OptionValue["ContextPath"], $Context = OptionValue["Context"]}, Compress[args]];
		res = TimeConstrained[
			Quiet[
				Check[qImport[makeMWAComputeURL[type, argscompressed], "String"],
					Message[msghead::nffil, Import]; $Failed,
					{MessageName[FetchURL, "conopen"], MessageName[FetchURL, "contime"], 
 					MessageName[FetchURL, "erropts"], MessageName[FetchURL, "httperr"], 
 					MessageName[FetchURL, "nofile"], MessageName[FetchURL, "nolib"], 
		...
		If[res =!= $Failed,
			Block[{$ContextPath = OptionValue["ContextPath"], $Context = OptionValue["Context"]},
				res = Quiet[Check[Uncompress[res, Hold], $Failed]]
			];
...
makeMWAComputeURL[type_, args_, opts___] := 
	Block[{baseurl, params},
		...
		baseurl = "Server" /. allMethodOptions[opts];
		baseurl = $AlphaQueryBaseURL[baseurl] <> $AlphaComputeJSP;
...
doEnterLinguisticAssistant[nb_, str_:""] := (
	NotebookWrite[nb, ToBoxes[AlphaIntegration`LinguisticAssistant[str, {}]], After];
