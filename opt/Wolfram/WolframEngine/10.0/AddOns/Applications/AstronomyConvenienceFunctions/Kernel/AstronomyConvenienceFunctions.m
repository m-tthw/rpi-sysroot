...
Which[(* Sunrise[] *)
      ...
      Length[argList] === 2,
       Which[(* good argument patterns *)
           ...
           MatchQ[argTypeList, {None, "location" | "date" | "applicationFunction" | "dateTime"}],
            {{}, {argList[[1]] -> {"loc"}}},
           MatchQ[argTypeList, {"location" | "date" | "applicationFunction" | "dateTime", None}],
            {{}, {argList[[2]] -> {"arg"}}},
           True,
            {{},{argList[[1]] -> {"locdate"}}}
           ],
...
getCoordinates[loc_Entity]:= Module[{polys, redpolys,geocode=EntityValue[loc, "Position"]/.gp_GeoPosition:>QuantityMagnitude[{Latitude[gp], Longitude[gp]}]},
	...
	redpolys=If[!MatchQ[polys, {}], Cases[
		Graphics`Mesh`SimplifyPolyLine[#, VerticesGoal -> 10]&/@Cases[{polys}, _Polygon, Infinity], 
			{_?NumberQ, _?NumberQ}, Infinity], polys];
...
iSunrise[args___, OptionsPattern[Sunrise]] := Module[{msam, dt, tz, igeolocation, ei, api, ipairs},
	...
	If[!Internal`RealValuedNumberQ[tz], Message[Sunrise::zone, tz];tz = $TimeZone];
	...
	If[MatchQ[msam[[1]], {{{_?(validLocationQ[#, Sunrise]&), _?(validDateQ[#, Sunrise]&)}..}, _, _}],
		ipairs = {preProcessCoordinates[#[[1]]], (#[[2]]/.DateObject[x___,opts:OptionsPattern[DateObject]]:>DateList[DateObject[x,opts]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x]}&/@(msam[[1, 1]]/.GeoPosition[x_]:>x);
	...
	dt=((msam[[1,2]]/.{start_?(validDateQ[#, Sunrise]&), end_?(validDateQ[#, Sunrise]&), incr_?(NumericQ[#] || (And @@ {NumericQ[#[[1]]], StringQ[#[[2]]]})&)}:>DateRange[start, end, incr])/.d_DateObject :> Block[{$TimeZone = DateValue[d, "TimeZone"]}, DateList[d]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x];
...
iSunset[args___, OptionsPattern[Sunset]] := Module[{msam, dt, tz, igeolocation, ei, api, ipairs},
	tz = OptionValue[TimeZone];
	ei = OptionValue[TimeDirection]/.{"Next"|"After"|Next|After->1, "Last"|"Previous"|"Before"|Previous|Last|Before->-1};
	If[!Internal`RealValuedNumberQ[tz], Message[Sunset::zone, tz];tz = $TimeZone];
	...
	dt=((msam[[1,2]]/.{start_?(validDateQ[#, Sunset]&), end_?(validDateQ[#, Sunset]&), incr_?(NumericQ[#] || (And @@ {NumericQ[#[[1]]], StringQ[#[[2]]]})&)}:>DateRange[start, end, incr])/.d_DateObject :> Block[{$TimeZone = DateValue[d, "TimeZone"]}, DateList[d]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x];
	If[!FreeQ[dt, $Failed|_$Failed|_DateRange], Return[$Failed]];
...
iSunPosition[args___, OptionsPattern[SunPosition]] := Module[{utz, tz, coord, igeolocation, msam, dt, api, ipairs, altm},
  ...
  If[!Internal`RealValuedNumberQ[tz], Message[SunPosition::zone, tz];tz = $TimeZone];
  ...
  If[msam[[1]]==={}, Return[$Failed]];
    If[MatchQ[msam[[1]], {{{_?(validLocationQ[#, SunPosition]&), _?(validDateQ[#, SunPosition]&)}..}, _}],
		ipairs = {preProcessCoordinates[#[[1]]], (#[[2]]/.d_DateObject :> DateList[d, TimeZone -> DateValue[d, "TimeZone"]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x]}&/@(msam[[1, 1]]/.GeoPosition[x_]:>x);
  ...
  dt=((msam[[1,2]]/.{start_?(validDateQ[#, SunPosition]&), end_?(validDateQ[#, SunPosition]&), incr_?(NumericQ[#] || (And @@ {NumericQ[#[[1]]], StringQ[#[[2]]]})&)}:>DateRange[start, end, incr])/. d_DateObject :> Block[{$TimeZone = DateValue[d, "TimeZone"]}, DateList[d]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x];
  If[!FreeQ[dt, $Failed|_$Failed|_DateRange], Return[$Failed]];
...
iMoonPosition[args___, OptionsPattern[MoonPosition]] := Module[{tz, coord, igeolocation, msam, dt, api, ipairs, altm},
  ...
  coord = OptionValue[CelestialSystem];
  altm = OptionValue[AltitudeMethod];
  If[!Internal`RealValuedNumberQ[tz], Message[MoonPosition::zone, tz];tz = $TimeZone];
  ...
  dt=((msam[[1,2]]/.{start_?(validDateQ[#, MoonPosition]&), end_?(validDateQ[#, MoonPosition]&), incr_?(NumericQ[#] || (And @@ {NumericQ[#[[1]]], StringQ[#[[2]]]})&)}:>DateRange[start, end, incr])/.d_DateObject :> DateList[d, TimeZone -> DateValue[d, "TimeZone"]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x];
  If[!FreeQ[dt, $Failed|_$Failed|_DateRange], Return[$Failed]];
...
iMoonPhase[] := Module[{tz, igeolocation, apiresult, api},
  tz = $TimeZone;
  If[!Internal`RealValuedNumberQ[tz], Message[MoonPhase::zone, tz];tz = $TimeZone];
...
iSolarEclipse[OptionsPattern[SolarEclipse]] := Module[{tz, igeolocation, apiresult, ei, et, api},
  ...
  If[!Internal`RealValuedNumberQ[tz], Message[SolarEclipse::zone, tz];tz = $TimeZone];
...
iSolarEclipse[{start_?(validDateQ[#, SolarEclipse]||validDateStringQ[#, SolarEclipse]&), end_?(validDateQ[#, SolarEclipse]||validDateStringQ[#, SolarEclipse]&), All}, opts:OptionsPattern[SolarEclipse]] := 
 Module[{tz, igeolocation, apiresult, ei, et, api},
  ...
  If[!MatchQ[et, Automatic|"Annular"|"Hybrid"|"Partial"|"Total"], Message[SolarEclipse::ectype, et];et=Automatic];
  igeolocation = preProcessCoordinates[$GeoLocation]/.GeoPosition[x_]:>x;
  api = ReleaseHold[APICompute[SolarEclipse,"AstronomyConvenienceFunction", {"SolarEclipse", "MaximumEclipseDate", igeolocation, DateRange[start, end, {2, "Week"}], ei, et, $TimeZone, tz, igeolocation, All}]];
  If[!FreeQ[api, $Failed["ComputationTimeout"]],
  	Message[SolarEclipse::time] 
  ];
   apiresult = If[!MatchQ[api, _Missing|$Failed|_$Failed], postProcessDateResults[#, tz, Automatic, Automatic]&/@("Result" /. api), $Failed]
  ]
...
iSolarEclipse[{start_?(validDateQ[#, SolarEclipse]||validDateStringQ[#, SolarEclipse]&), end_?(validDateQ[#, SolarEclipse]||validDateStringQ[#, SolarEclipse]&), All}, prop:"GraphicsData"|"PartialPhasePolygon"|"TotalPhasePolygon"|"TotalPhaseCenterLine"|"Type", opts:OptionsPattern[SolarEclipse]] := Module[{tz, igeolocation, apiresult, ei, et, api},
  ...
  If[!MatchQ[et, Automatic|"Annular"|"Hybrid"|"Partial"|"Total"], Message[SolarEclipse::ectype, et];et=Automatic];
  igeolocation = preProcessCoordinates[$GeoLocation]/.GeoPosition[x_]:>x;
  api = ReleaseHold[APICompute[SolarEclipse,"AstronomyConvenienceFunction", {"SolarEclipse", prop, igeolocation, DateList/@DateRange[start, end, {2, "Week"}], ei, et, $TimeZone, tz, igeolocation, All}]];
  If[!FreeQ[api, $Failed["ComputationTimeout"]],
  	Message[SolarEclipse::time] 
  ];
   apiresult = If[!MatchQ[api, _Missing|$Failed|_$Failed], 
...
SolarEclipse[args___, opts:OptionsPattern[SolarEclipse]] /; (ArgumentCountQ[SolarEclipse,Length[{args}],0,2]) := Block[{res},
...
iLunarEclipse[OptionsPattern[LunarEclipse]] := Module[{tz, igeolocation, apiresult, ei, et, api},
  ...
  ei = OptionValue[TimeDirection]/.{"Next"|"After"|Next|After->1, "Last"|"Previous"|"Before"|Previous|Last|Before->-1};
  et = OptionValue[EclipseType];
  If[!Internal`RealValuedNumberQ[tz], Message[LunarEclipse::zone, tz];tz = $TimeZone];
...
iLunarEclipse[{start_?(validDateQ[#, LunarEclipse]||validDateStringQ[#, LunarEclipse]&), end_?(validDateQ[#, LunarEclipse]||validDateStringQ[#, LunarEclipse]&), All}, opts:OptionsPattern[LunarEclipse]] := Module[{tz, igeolocation, apiresult, ei, et, api},
  ...
  If[!MatchQ[et, Automatic|"Partial"|"Total"], Message[LunarEclipse::ectype, et];et=Automatic];
  igeolocation = preProcessCoordinates[$GeoLocation]/.GeoPosition[x_]:>x;
  api = ReleaseHold[APICompute[LunarEclipse,"AstronomyConvenienceFunction", {"LunarEclipse", "MaximumEclipseDate", igeolocation, DateList/@DateRange[start, end, {2, "Week"}], ei, et, $TimeZone, tz, igeolocation, All}]];
  If[!FreeQ[api, $Failed["ComputationTimeout"]],
  	Message[LunarEclipse::time] 
  ];
   apiresult = If[!MatchQ[api, _Missing|$Failed|_$Failed], postProcessDateResults[#, tz, Automatic, Automatic]&/@("Result" /. api), $Failed]
...
iLunarEclipse[{start_?(validDateQ[#, LunarEclipse]||validDateStringQ[#, LunarEclipse]&), end_?(validDateQ[#, LunarEclipse]||validDateStringQ[#, LunarEclipse]&), All}, prop:"MaximumEclipseDate"|"TotalPhaseStartDate"|"TotalPhaseEndDate"|"TotalPhaseStartDate"|"PartialPhaseStartDate"|"PartialPhaseEndDate", opts:OptionsPattern[LunarEclipse]] := Module[{tz, igeolocation, apiresult, ei, et, api},
  ...
  If[!MatchQ[et, Automatic|"Partial"|"Total"], Message[LunarEclipse::ectype, et];et=Automatic];
  igeolocation = preProcessCoordinates[$GeoLocation]/.GeoPosition[x_]:>x;
  api = ReleaseHold[APICompute[LunarEclipse,"AstronomyConvenienceFunction", {"LunarEclipse", prop, igeolocation, DateList/@DateRange[start, end, {2, "Week"}], ei, et, $TimeZone, tz, igeolocation, All}]];
  If[!FreeQ[api, $Failed["ComputationTimeout"]],
  	Message[LunarEclipse::time] 
  ];
  apiresult = If[!MatchQ[api, _Missing|$Failed|_$Failed], 
   	postProcessDateResults[#, tz, Automatic, Automatic]&/@("Result" /. api), $Failed]
  ]
...
LunarEclipse[args___, opts:OptionsPattern[LunarEclipse]] /; (ArgumentCountQ[LunarEclipse,Length[{args}],0,2]) := Block[{res},
...
iSiderealTime[args___] := Module[{tz, igeolocation, msam, dt, api, ipairs},
  tz = $TimeZone;
  If[!Internal`RealValuedNumberQ[tz], Message[SiderealTime::zone, tz];tz = $TimeZone];
  ...
  dt=((msam[[1,2]]/.{start_?(validDateQ[#, SiderealTime]&), end_?(validDateQ[#, SiderealTime]&), incr_?(NumericQ[#] || (And @@ {NumericQ[#[[1]]], StringQ[#[[2]]]})&)}:>DateRange[start, end, incr])/.d_DateObject :> Block[{$TimeZone = DateValue[d, "TimeZone"]}, DateList[d]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x];
  If[!FreeQ[dt, $Failed|_$Failed|_DateRange], Return[$Failed]];
...
iDaylightQ[args___, OptionsPattern[DaylightQ]] := Module[{tz, igeolocation, msam, dt, api, ipairs, sunangletest},
  ...
  If[!Internal`RealValuedNumberQ[tz], Message[DaylightQ::zone, tz];tz = $TimeZone];
  ...
  dt=((msam[[1,2]]/.{start_?(validDateQ[#, DaylightQ]&), end_?(validDateQ[#, DaylightQ]&), incr_?(NumericQ[#] || (And @@ {NumericQ[#[[1]]], StringQ[#[[2]]]})&)}:>DateRange[start, end, incr])/.d_DateObject :> Block[{$TimeZone = DateValue[d, "TimeZone"]}, DateList[d]])/.x_String/;MatchQ[Quiet[DateList[x]], _List]:>DateList[x];
  If[!FreeQ[dt, $Failed|_$Failed|_DateRange], Return[$Failed]];
