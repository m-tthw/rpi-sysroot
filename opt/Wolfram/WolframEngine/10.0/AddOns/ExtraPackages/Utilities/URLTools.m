...
FetchURL::nolib = "THe HTTPClient library could not be located or initialized."
...
iFetchURL[method:("GET" | "POST"), url_String, outFile1_String, timeout_, verifyPeer_,
    ...
    Module[{outFile = outFile1, requestHeaders, requestParams, fetchResult,
        ...
        (* HTTPClient code takes only Rule, not RuleDelayed, so fix the unlikely case where a user used RuleDelayed. *)
        requestHeaders = requestheaderfields /. RuleDelayed->Rule;
        requestParams = requestparameters /. RuleDelayed->Rule;
        
        (* Headers as a result type are only relevant for HTTP, and HTTPClient will give errors in they
