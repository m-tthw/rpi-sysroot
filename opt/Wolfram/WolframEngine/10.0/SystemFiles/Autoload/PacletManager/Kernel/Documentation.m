...
(* These are the functions from StartUp/Documentation.m that we add defs for.
   ...
   (There is also the GetVirtualCellGroup function, which might be deprecated in the very near future).
*)
...
GetVirtualCellGroup[uri_String] :=
    Module[{language, result},
        ...
        *)
        result = Quiet[resolveURL[uri, language, {Null, Null} (*ignored*), Null&, False, False, False]];
        If[ListQ[result],
            {First[result], ""},
        (* else *)
...
resolveURL[url_String, language_String, feData_, completionFunc_,
    ...
    Module[{trimmedURL, resource, resourceWithoutFragment, wordsOnly, fallThroughToSearch,
                fragPart, parts, linkBase, resourceName, context, pathOrURL,
                    symbolNamePieces, directHitResult, searchTextTranslated, directHitURI, searchHeaderData},
        ...
        *)
        ...
        Which[
            ...
            language != "English",
                ...
                resolveDocResource[linkBase, context, resourceName, "English", localFilesOnly],
            True,
                (* Not found. *)
                {Null, Null}
        ]
