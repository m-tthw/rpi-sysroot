    ...
    Module[{task, async, siteRec},
        ...
        *)
        ...
        If[Head[task] === AsynchronousTaskObject,
            ...
            If[async === False,
                CheckAbort[
                    Quiet @ WaitAsynchronousTask[task],
                (* On abort: *)
                    Quiet[
...
finishPacletSiteUpdate[task_] :=
    Module[{siteURL, file, statusCodeOrErrorString, success, interactive, async, data, sitePaclets, errorMsg},
        {siteURL, file, interactive, async, statusCodeOrErrorString} = getTaskData[task];
        freeTaskData[task];
        success = True;
