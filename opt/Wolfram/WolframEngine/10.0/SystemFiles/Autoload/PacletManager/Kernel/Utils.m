...
storeInCache[kernelVersion_String, versionSpec_String, key_String] :=
    Block[{matches, components, specComponents, kernelComponents},  (* Block for speed only *)
        ...
        If[Length[components] > 1,
            If[Or @@ (kernelVersionMatches[kernelVersion, StringTrim[#]]& /@ components),
                matches = True
            ],
        (* else *)
            Which[
...
BuildPacletSiteFiles[dir_String, fileList:_List:{}, OptionsPattern[]] :=
    Module[{files, vbose, wri, mVersionSuffix, layoutPaclets},
        ...
        (* On WRI server we have several variants of the PacletSite.bin/mz/m files.
           ...
           Note that this version of the PM can only build a PacletSite_9x.m, as it creates version-specific files
        ...
        *)
        If[wri && StringMatchQ[mVersionSuffix, "9*"],
            (* Possibly temporary: Cull out all doc paclets for a V9 version of the site file, as we are not using doc paclets atm.
               If we take this culling step out, and thus consider doc paclets, add "LayoutDocs" to the PCfindMatching call below.
            *)
            files = Select[files, !StringMatchQ[#, "*SystemDocs*"]&];
            (* When building a version-specific PacletSite.m file, we want to exclude paclets that are older than ones in
               the layout for this version of M. For example, we don't want to include a paclet that was an update to a V8
               in-layout paclet, and marked with MathematicaVersion->8+, in PacletSite_9.m, because the V9 layout will
               have a version at least as new. Because we are doing this culling based on the current layout contents,
               we must use a version of M that matches the PacletSite.m file that we are being asked to build.
            *)
            If[versionCompare[mVersionSuffix, getKernelVersionString[]] =!= Equal,
                Print["WARNING: An incorrect version of Mathematica is being used to build this PacletSite.m file"];
                Print["You must run a version that is identical to the value passed in for the MathematicaVersion option."]
            ];
            layoutPaclets = PCfindMatching["Collections" -> {"Layout"}];
            files = Select[files,
                        If[keepPaclet[#, layoutPaclets],
                            True,
                        (* else *)
                            Print["**** File " <> # <> " is being excluded from PacletSite_" <> mVersionSuffix <>
                                  " because it is not newer than a paclet already in the layout for that version of Mathematica."];
                            False
                        ]&
                    ];
...
buildSiteFiles[files:{___String}, outDir_String, mVersionSuffix_String, verbose:(True | False)] :=
    Module[{pacletFile, siteFile, strm, pacletWritingFunction},
        ...
        pacletWritingFunction = If[StringMatchQ[mVersionSuffix, "_9*"], writePaclet9, writePacletCompatible];
...
writePacletCompatible[strm_OutputStream, pacletFile_String] :=
    Module[{dataBuffer, piString, pacletStartPos},
        dataBuffer = ZipGetFile[pacletFile, "PacletInfo.m"];
        If[!ListQ[dataBuffer],
...
(* Decides whether to keep the given paclet, based on whether it is newer than a corresponding one in the layout. *)
keepPaclet[pacletFile_String, layoutPaclets_List] :=
    Module[{p, layoutPaclet, name},
        p = CreatePaclet[pacletFile];
        name = p["Name"];
        layoutPaclet = If[Length[#] > 0, First[#], Null]& @ Select[layoutPaclets, #["Name"] == name &];
        (* Keep any paclet that doesn't have the same name as a local one, or if it does match a local one
           but the local one is not installed in the Mathematica layout.
        *)
        If[layoutPaclet === Null,
            Return[True]
        ];
        (* Reject a paclet that is not newer than the matching one in the layout. *)
        If[!PacletNewerQ[p["Version"], layoutPaclet["Version"]],
            Return[False]
        ];
        True
    ]
