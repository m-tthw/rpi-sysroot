...
WolframAlpha[str_String, "URL", opts:OptionsPattern[]] := 
	Block[{baseurl, baseparams, async, format, assumption, includepodid, excludepodid,
			...
			languagecode, userip, rawparameters, params, methodOptions},
...
WolframAlpha[str_String, "CDFURL", opts:OptionsPattern[]] := 
	Block[{baseurl, assumption, rawparameters, params, methodOptions},
		...
		baseurl = Switch[ ToLowerCase[baseurl],
				...
				"test2", "http://www.test2.wolframalpha.com/cdf/",
...
WolframAlpha[str_String, "MathematicaParseURL" | "WolframParseURL", opts:OptionsPattern[]] := 
	Block[{baseurl, assumption, msessinfo, rawparameters, params, methodOptions},
...
getTimeStamp[] := StringReplace[
	ExportString[DateString[DateList[],$TimeStampDateFormat,TimeZone->0],
		"Base64"],
"\n"->""](*adds a trailing \n for some reason...*)
...
csceResources[$entityPropertyContextPat] := {
	...
	"AssumptionsIcon" -> bitmapResource["LinguisticChoicesIcon", BaselinePosition->Scaled[0.1]],
...
csceResources[Automatic] :=	{
	...
	"AssumptionsIcon" -> bitmapResource["LinguisticAssumptionsIcon", BaselinePosition->Scaled[0.1]],
...
With[{boxversion = If[contextinfo === Automatic, 1, 3]},
	System`DynamicNamespace[
		"LinguisticAssistant",
		DynamicModule[{query = str, boxes = "None", allassumptions = {}, assumptions = chosenAssumptions, open = {1}},
			doNewFastParse[query, contextinfo, Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open]];
			If[boxversion === 1,
				Dynamic[
					...
					TrackedSymbols :> {query, boxes, allassumptions, assumptions, open}
				],
				Dynamic[
					AlphaIntegration`LinguisticAssistantBoxes[str, boxversion, contextinfo, Dynamic[query], Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open]],
					TrackedSymbols :> {query, boxes, allassumptions, assumptions, open}
				]
			],
			Evaluate @ If[BoxForm`sufficientVersionQ[10], UndoTrackedVariables :> {open}, Unevaluated[Sequence[]]]
		],
		BaseStyle -> {"Deploy"},
		Editable -> False,
		DeleteWithContents -> True,
...
DynamicModule[{emphasizeErrors = False},
	Panel[
		...
		If[$CloudControlEqual && MemberQ[open, 3],
			Column[{#, linguisticAssumptionsDisplay[contextinfo, Dynamic[query], Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open]]}, BaselinePosition -> {1,1}],
			#
		]& @ 
		...
		Which[
			...
			Grid[{{
				...
				Pane[
					linguisticTopInputField[contextinfo, Dynamic[query], Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open]],
					BaselinePosition -> Baseline,
			...
			Grid[{Flatten @ {
				linguisticEditSwitchFromError[query, contextinfo, Dynamic[open]],
				EventHandler[
					Pane[
						linguisticTopInputField[contextinfo, Dynamic[query], Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open]],
						BaselinePosition -> Baseline,
				...
				linguisticAssumptions[contextinfo, Dynamic[query], Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open]]
			}}, Alignment -> If[$CloudControlEqual, {Center, Center}, Baseline], BaselinePosition -> {1,2}, Spacings -> 0.3],
		...
		If[$CloudControlEqual,
			...
			Appearance :> FEPrivate`If[
				FEPrivate`SameQ[emphasizeErrors, True],
				FEPrivate`FrontEndResource["WAExpressions", "ControlEqualErrorAppearance"],
				FEPrivate`FrontEndResource["WAExpressions", "ControlEqualAppearance"]
			]
		],
		BaselinePosition -> Baseline,
		DefaultBaseStyle -> {}
...
linguisticAssumptionsDisplay[contextinfo_, Dynamic[query_], Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_]] :=
(
	...
	HoldPattern[updateWithAssumptions][nb_, assumptioninputs_, Dynamic[q_], Dynamic[opts_]] :> 
		updateWithAssumptionsLinguisticAssistant[query, contextinfo, Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open], assumptioninputs]
) /; $CloudControlEqual


linguisticAssumptions[contextinfo_, Dynamic[query_], Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_]] := 
	If[TrueQ[$LinguisticAssistantCompact], {}, RawBoxes[""]] /; allassumptions === {}


linguisticAssumptions[contextinfo: $cscePat, Dynamic[query_], Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_]] :=
	Tooltip[
		ActionMenu[
			csceResource["AssumptionsIcon", contextinfo],
	...
	Tooltip[
		Button[
			If[TrueQ[$LinguisticAssistantCompact],
				bitmapResource["LinguisticAssumptionsIcon", BaselinePosition -> Scaled[0.1]],
...
updateWithAssumptionsLinguisticAssistant[query_, contextinfo_, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_], assumptioninputs:{__}] := 
(
...
linguisticEditSwitchFromError[query_, contextinfo_, Dynamic[open_]] := 
	Tooltip[
		If[$CloudControlEqual,
			Button[
				Pane[
	...
	Tooltip[
		Button[
			Column[{
				...
				}, Spacings -> If[$CloudControlEqual, 0.05, -0.2], Alignment -> Center, Background -> White
			],
			open = {1,2};
...
linguisticTopInputField[contextinfo_, Dynamic[query_], Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_]] := 
	EventHandler[
		InputField[
			Dynamic[query, (query = #; doNewFastParse[query, contextinfo, Dynamic[boxes], Dynamic[allassumptions], Dynamic[assumptions], Dynamic[open]])&],
			String,
			Appearance -> None,
...
doNewFastParse[query_, contextinfo: $cscePat, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_]] :=
	Block[{rawparse},
		rawparse = TimeConstrained[CSCEParse[query, contextinfo], 5, $TimedOut];
		Switch[rawparse,
			{__},
...
(* Experiment with using a background call to query.jsp *)
doNewFastParse[query_, contextinfo_, Dynamic[boxes_], Dynamic[allassumptions_], Dynamic[assumptions_], Dynamic[open_]] :=
