...
realInstallation[path_String /; File === Quiet[FileType[path]]] := 
	(* path is like /opt/intel/Compiler/11.1/064/bin/intel64/icc *)
...
realInstallation[path_String /; Directory === Quiet[FileType[path]]] := 
	(* path is one of these:
		...
		If not one of these, we can't handle it
	*)
	With[{plusicc = FileNameJoin[{path, "icc"}],
		plusintel64icc = FileNameJoin[{path, "intel64", "icc"}],
		plusia32icc = FileNameJoin[{path, "ia32", "icc"}]},
		Which[
			validCompilerCommandQ[plusicc], FileNameDrop[path, -2],
			validCompilerCommandQ[plusintel64icc] ||
				validCompilerCommandQ[plusia32icc], FileNameDrop[path, -1],
			True, path
		]
	]
