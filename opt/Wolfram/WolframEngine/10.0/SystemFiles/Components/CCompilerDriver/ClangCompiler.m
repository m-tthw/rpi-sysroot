		...
		"SystemCompileOptions" -> 
			...
			Join[{"-fPIC", $OptFlag},
				If[$OperatingSystem === "MacOSX",
					{"-mmacosx-version-min=10.6","-framework", "Foundation"},
					{}
				]
			]
	}
...
(* Adjsut the standard C++ libraries for OSX *)
...
ClangCompiler["CreateLibraryCommands"][
	...
	Module[{},
		...
		CommandJoin[
			...
			" ", libpath,
			" ", formatLibraries[syslibs, libs, False], 
...
ClangCompiler["CreateExecutableCommands"][
	...
	Module[{},
		...
		CommandJoin[
			...
			" ", QuoteFiles[extraObjects], 
			" ", libpath,
			" ", formatLibraries[syslibs, libs, True], 
...
mathlinkframeworkOptionQ[str_String] := StringMatchQ[str, StartOfString ~~ "-framework" ~~ ___~~ "mathlink"];

formatLibraries[libs_List, execQ_] :=(
	(* Ensure that we never receive Null in the List *)
	DeleteCases[Riffle[formatLibrary[#,execQ]& /@ libs, " "], Null]
);

formatLibraries[libs_List, libs2_List, execQ_] := formatLibraries[Join[libs, libs2], execQ]

formatLibrary[lib_?mathlinkframeworkOptionQ, execQ_] := (
	If[execQ === False,
...
formatLibrary[lib_?frameworkOptionQ, _] := "-framework "<>QuoteFile[StringTrim[StringReplace[lib,"-framework"->""]]];

formatLibrary[lib_, _] := (
	If[LibraryPathQ[lib], 
