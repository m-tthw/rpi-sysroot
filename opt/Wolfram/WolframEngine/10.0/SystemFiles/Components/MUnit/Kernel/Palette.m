...
$exclusionForms = Complement[Append[Names["*Form"], "Print"], {"CapForm", "EdgeForm", "ExportForm", "FaceForm", "HornerForm", "JoinForm", "PrecedenceForm", "RealBlockDiagonalForm", "ResponseForm"}]
...
ExportForm, FaceForm, HornerForm, JoinForm, PrecedenceForm, RealBlockDiagonalForm and ResponseForm cannot be converted into tests.", WindowSize -> {400, 140}]]
	]
	...
	Module[{gt, presetMessageOptionsValues = (MessageOptions /. Options[$FrontEnd, MessageOptions]), newMessageOptionsValues, cs, resultcells, cellids, styleList},
		...
		Catch[If[($formCheck=!=False)&&(Cases[DeleteCases[gt[[1]], RowBox[{"(*", __, "*)"}], Infinity], Alternatives @@ MUnit`$exclusionForms, Infinity] =!= {}), 
			Throw[CurrentValue[nb, WindowTitle] = If[Cases[NotebookInformation[nb], _["FileName", _]]==={},
				...
				CurrentValue[nb, ShowSelection] = Inherited;
				MessageDialog["Expressions with symbols in the list given by Names[\"*Form\"] or Print with the exception of CapForm, EdgeForm, ExportForm, FaceForm, HornerForm, JoinForm, PrecedenceForm, RealBlockDiagonalForm and ResponseForm cannot be used in tests.", WindowFrame -> "ModalDialog"]]];
