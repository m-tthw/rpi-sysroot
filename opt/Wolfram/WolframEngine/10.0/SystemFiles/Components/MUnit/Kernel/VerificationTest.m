	...
	Block[{parse,args,o,res,input},
		...
		(
			...
			res = Switch[Length[args],
					1, iVerificationTest[Extract[args,{1},Hold],True,{},ReleaseHold@o],
...
iVerificationTest[Hold[input_], expected_:True, Shortest[expectedMsgs_:{}], opts:OptionsPattern[]] := 
	Module[{munitResult,newExpectedMsgs},
		With[{defaultOptions=Options[VerificationTest], newOptions=Quiet@SetOptions[VerificationTest,opts]},
			...
			munitResult = MUnit`Test[input, ReleaseHold@expected, ReleaseHold@newExpectedMsgs, newOptions];
				...
				With[{ntr = TestResultObjectToAssociation[tr]},
					...
					If[verifCellIDAbove =!= Missing["NotFound"]
						...
						If[MatchQ[cellGroupStylesInPresentGroup, Alternatives @@ testCellGroupStyleLists]
							...
							containsErrorBoxQ = (Module[{a},Cases[Quiet[ReleaseHold[(Hold@MakeExpression[a, StandardForm] /. a -> #) & /@ (BoxData[If[ListQ@#,
