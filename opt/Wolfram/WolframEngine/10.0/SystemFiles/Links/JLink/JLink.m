...
`Package`$jlinkDir = DirectoryName[FindFile[$Input]]
...
(* This allows natural syntax for setting Java fields: obj@field = val.
   The change to Set must be done outside the .mx file because it wouldn't be captured in the .mx file.
*)
If[$VersionNumber < 8,
	`Private`prot = Unprotect[Set];
	Set[`Private`sym_Symbol[`Private`arg_Symbol], `Private`val_] :=
	    With[{`Private`obj = `Private`sym},
	        JLink`CallJava`Private`setField[`Private`obj[`Private`arg], `Private`val] /; Head[`Private`obj] === Symbol && StringMatchQ[Context[`Private`obj], "JLink`Objects`*"]
	    ];
	Protect[Evaluate[`Private`prot]],
(* else *)
    (* In Version 8 we can dispense with the special Set rule because there is a new internal system. We register a VetoableValueChange
       handler. The jlinkVetoFunction will be called whenever obj@field = val is called. We evaluate setField,
       which does the Java work, as a side-effect and then return False to prevent the Mathematica-side assignment from doing anything.
    *)
    ...
    `Private`jlinkVetoFunction[HoldComplete[`Private`sym_Symbol, `Private`lhs_, `Private`rhs_, _, DownValues]] :=
        (JLink`CallJava`Private`setField[`Private`lhs, `Private`rhs]; False) /; StringMatchQ[Context[`Private`sym], "JLink`Objects`*"]
]
