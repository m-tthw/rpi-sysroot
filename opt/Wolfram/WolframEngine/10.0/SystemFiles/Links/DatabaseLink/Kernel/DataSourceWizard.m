...
connectionDialog[pageIn_Integer, type_String, url_, o:OptionsPattern[]] := CreateDialog[DynamicModule[
    ...
    refreshJDBCInfo[] := (
        jdbcNames = Sort@JDBCDriverNames[];
    ...
    connectionContents[5] := connectionContentsTemplate[p["Type"] <> " Connection",
        ...
        (*Print@InputForm@(p[#] & /@ {"Type", "Database", "Username", "Password"});*)
                	...
                	Checkbox[
                		Dynamic[p["RelativePath"], (
                            ...
                            p["Database"] = If[TrueQ@p["RelativePath"],
                                FileNameJoin[With[{path = absToRel[p["Database"], p["Location"]]},
                                    {DirectoryName@path, altFileBaseName@path}]],
                                (* else *)
                                ExpandFileName[FileNameJoin[{DirectoryName[p["Location"] /. Null -> ""], p["Database"]}]]
                            ]
                ...
                {Style["Relative Path", Larger],
                    Checkbox[
                        Dynamic[p["RelativePath"], (
                            p["RelativePath"] = #;
                            p["Database"] = If[TrueQ@p["RelativePath"],
                                With[{path = absToRel[p["Database"], p["Location"]]}, path],
                                (* else *)
                                ExpandFileName[FileNameJoin[{DirectoryName[p["Location"] /. Null -> ""], p["Database"]}]]
                            ]
                        ) &], BaselinePosition -> Scaled[0.2]]},
 ...
 * There is some logic based on driver class in SQL.m; see SQLConnectionUsableQ.
 *)
